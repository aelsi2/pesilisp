...
static result_t read_cond_clause(object_t *clause, env_t *env,
    ...
    if (list.count == 1) {
        result_t result = obj_eval(list.array[0], env, recursion_limit, dirty);
        list_end(list);
        *match = true;
        return result;
    }

    result_t cond_result = obj_eval(list.array[0], env, recursion_limit, dirty);
    ...
    *match = true;
    ...
    for (int i = 1; i < list.count; i++) {
        obj_unref(result);
        result_t eval_res =
            obj_eval(list.array[i], env, recursion_limit, dirty);
...
static result_t lisp_if(object_t *func, object_t *args, env_t *env,
    ...
    if (obj_is_null(list.array[0])) {
        if (list.count == 2) {
            ...
            arg_eval(list, 2, env, recursion_limit, dirty);
            result = obj_ref(list.array[2]);
        }
    } else {
        arg_eval(list, 1, env, recursion_limit, dirty);
