...
static error_t *read_let_list(object_t *args, env_t *source, env_t *target,
    ...
    for (int i = 0; i < list.count; i++) {
        ...
        result_t eval_result =
            obj_eval(def.array[1], source, recursion_depth, dirty);
...
static result_t lisp_let(object_t *func, object_t *args, env_t *env,
    ...
    error_t *error =
        read_let_list(list.array[0], env, child_env, recursion_depth, dirty);
    if (error) {
        env_free(child_env);
        list_end(list);
    ...
    for (int i = 1; i < list.count; i++) {
        obj_unref(result);
        result_t eval_res =
            obj_eval(list.array[i], child_env, recursion_depth, dirty);
...
static result_t lisp_let_star(object_t *func, object_t *args, env_t *env,
    ...
    error_t *error = read_let_list(list.array[0], child_env, child_env,
                                   recursion_depth, dirty);
    if (error) {
        env_free(child_env);
        list_end(list);
        return result_error(error);
...
static result_t lisp_apply(object_t *func, object_t *args, env_t *env,
    ...
    result_t result = obj_call_func(list.array[0], list.array[1], env,
                                    recursion_depth, dirty);
    list_end(list);
